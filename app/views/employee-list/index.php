...
<div class="modal fade text-dark" id="myModalInfo" aria-hidden="true">
    <div class="modal-dialog modal-lg position-relative">
        <div class="modal-content text-start">
            ...
            <!-- UPDATED MODAL HEADER WITH FOR ID BUTTON -->
            <div class="modal-header">
                <h5 class="modal-title" id="profileModalLabel">Profile Details</h5>
                <div class="d-flex align-items-center gap-2">
                    <button type="button" id="btnForId" class="btn btn-sm fw-bold border border-2" onclick="toggleForId()">
                        <i class="fas fa-id-card me-1"></i><span id="btnForIdLabel">For ID: Pending</span>
                    </button>
...
<script>
    ...
    // FOR ID TOGGLE LOGIC
    // ==========================================
    let currentForIdStatus = 0;

    function fn_getForIdStatus(empId) {
        const btn = document.getElementById('btnForId');
        btn.disabled = true;

        const formData = new FormData();
        formData.append('employeeid', empId);

        fetch('model/employee/get-forid-status.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
        .then(data => {
            currentForIdStatus = parseInt(data.forid) || 0;
            updateForIdButton(currentForIdStatus);
            btn.disabled = false;
        })
        .catch(error => {
            console.error('Error fetching ForID status:', error);
            btn.disabled = false;
        });
    }

	    function updateForIdButton(status) {
	    const btn = document.getElementById('btnForId');
	    if (!btn) return;
	    ...
	    if (status === 1) {
	        ...
	        btn.innerHTML = '<i class="fas fa-id-card me-1"></i><span id="btnForIdLabel">For ID: Sent</span>';
	    } else {
        btn.classList.remove('btn-primary');
        btn.classList.add('btn-outline-secondary');
        btn.innerHTML = '<i class="fas fa-id-card me-1"></i><span id="btnForIdLabel">For ID: Pending</span>';
    		}
		}		

    function toggleForId() {
    const empId = document.getElementById('currentEmpId').value;
    const newStatus = currentForIdStatus === 1 ? 0 : 1;

    // Only validate when marking as Sent (1), not when reverting to Pending (0)
	    if (newStatus === 1) {
	    ...
	    const address      = document.getElementById('birthplaceInput').value.trim();
	    const iceName      = document.getElementById('iceName').value.trim();
	    const iceRel       = document.getElementById('iceRelationship').value.trim();
	    const iceNum       = document.getElementById('iceNumber').value.trim();

	    // Check ID table has at least 1 row
	    const idTableRows = document.querySelectorAll('#idTableBody tr');
	    const hasValidId  = idTableRows.length > 0 && 
	                        !(idTableRows.length === 1 && idTableRows[0].querySelector('td[colspan]'));

	    const missing = [];
	    if (!fullname)    missing.push('Full Name');
	    if (!nickname)    missing.push('Nickname');
	    if (!designation) missing.push('Designation');
	    if (!office)      missing.push('Office');
	    if (!address)     missing.push('Address');
	    if (!iceName)     missing.push('ICE Contact Name');
	    if (!iceRel)      missing.push('ICE Relationship');
	    if (!iceNum)      missing.push('ICE Contact Number');
	    if (!hasValidId)  missing.push('At least 1 Identification Card');

	    if (missing.length > 0) {
	        alert('Please complete the following before marking as Sent:\n\n• ' + missing.join('\n• '));
	        return;
		    }
	}

    const actionText = newStatus === 1 ? 'Mark as Sent for ID?' : 'Mark as Pending?';
    if (!confirm(actionText)) return;
        ...
        const formData = new FormData();
        formData.append('employeeid', empId);
        formData.append('forid', newStatus);

        fetch('model/employee/update-forid.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
        .then(data => {
            currentForIdStatus = newStatus;
            updateForIdButton(newStatus);
            btn.innerHTML = '<i class="fas fa-id-card me-1"></i> <span id="btnForIdLabel">' + (newStatus === 1 ? 'For ID: Sent' : 'For ID: Pending') + '</span>';
            btn.disabled = false;
        })
        .catch(error => {
            console.error('Error updating ForID:', error);
            alert('Failed to update For ID status.');
            btn.innerHTML = '<i class="fas fa-id-card me-1"></i> <span id="btnForIdLabel">' + (currentForIdStatus === 1 ? 'For ID: Sent' : 'For ID: Pending') + '</span>';
            btn.disabled = false;
        });
    }
</script>
