<?php
	...
	if (!isset($_GET['id']) || empty(trim($_GET['id']))) {
		...
		exit;
	} elseif (!isset($_GET['deviceid']) || empty(trim($_GET['deviceid']))) {
		http_response_code(400);
		echo json_encode([
			"status" => "error",
			"message" => "Missing or empty 'Device ID Code' parameter."
		]);
		exit;
	} elseif (!isset($_GET['devicename']) || empty(trim($_GET['devicename']))) {
		http_response_code(400);
		echo json_encode([
			"status" => "error",
			"message" => "Missing or empty 'Device Name' parameter."
		]);
		exit;
	...
	$devicename = htmlspecialchars(trim($_GET['devicename']), ENT_QUOTES, 'UTF-8');
	...
	try {
		...
		if ($stmt->rowCount() > 0) {
			...
			if ( $mesecured == "0a7a004339f450a46fe7b34767c54577") {
				$update_query = "UPDATE employee_tbl SET pinword=:new_pincode, device_id=:deviceid, device_name=:devicename WHERE emp_idcode = :id";
				...
				$update_stmt->bindParam(":deviceid", $deviceid);
				$update_stmt->bindParam(":devicename", $devicename);
